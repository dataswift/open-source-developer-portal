...
description: Step 3 to authenticating HAT applications on the iOS platform
---
...
* `success` is a callback to execute when the library have successfully authorized user. You can use this callback to dismiss `SFSafariViewController`, save the values needed and navigate to the next `UIViewController`
* `failed` is a callback to execute when the library couldn't authorize user. You can use this callback to dismiss `SFSafariViewController` and show an error message to the user

Having received the success callback you have to save the token, **ALWAYS** use the `Keychain` to save the token. **DO NOT** save it in `UserDefaults` or in any non encrypted database. One very nice library to use in order to write/read on `Keychain` is [Keychain-swift](https://github.com/evgenyneu/keychain-swift). A good idea would be to also save the full `HAT Address` as well, as you are gonna need it many times in order to communicate with the HAT.

As soon as you save the token you are free to navigate to your next `UIViewController`.

# Fail callback

In case the authentication failed, you will redirected to the fallback url. In that case you should dismiss `SFSafariViewController` and then update the UI or show an error message back to the user.

In order to dismiss safari you do it from the viewController that presented originally the `SFSafariViewController`, like this:

```swift
self.safariVC?.dismiss(animated: true, completion: nil)
self.safariVC?.removeFromParent()
```
